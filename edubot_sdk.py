...
class EdubotGCS:  # mavwifi.Wifi
    ...
    def _create_connection(self, connectionMethod, ip, port, device, baud):
        ...
        if connectionMethod not in self.ConnectionMethod:
            logging.error(
                f"{self.name}:Connection: Unknown connection method: {connectionMethod}")
        ...
        try:
            if connectionMethod == self.ConnectionMethod.serial:
                ...
                mav_socket = mavutil.mavlink_connection(
                    '%s:%s:%s' % (connectionMethod.name, ip, port))

            return mav_socket

        except socket.error as e:
            logging.error(
                f"{self.name}:Connection: Can not connect to robot: {e}")
        ...
        try:
            while True:
                ...
                if confirm >= self.__mav_settings.mavlink_send_number:
                    logging.debug(
                        f"[{self.name}] <Ack> {command_name}, result: {self.__MavResult.SEND_TIMEOUT.name}")
                    return False
        finally:
            ...
            for confirm in range(self.__mav_settings.mavlink_send_number):
                ...
                if event.is_set():
                    ...
                    if msg.type_mask == mask:
                        logging.debug(
                            f"[{self.name}] <Ack> {command_name}, result: {self.__MavResult.ACCEPTED.name}")
                    else:
                        logging.debug(
                            f"[{self.name}] <Ack> {command_name}, result: {self.__MavResult.DENIED.name}")
                    return True
    ...
    def _send_yaw_orientation(self, yaw):

        target_system = self.mavlink_socket.target_system
        target_component = self.mavlink_socket.target_component

        event = threading.Event()
        self.wait_msg['GIMBAL_DEVICE_SET_ATTITUDE'] = event
        try:
            for confirm in range(self.__mav_settings.mavlink_send_number):
                self.mavlink_socket.mav.set_attitude_target_send(0, target_system, target_component, mavutil.mavlink.ATTITUDE_TARGET_TYPEMASK_THROTTLE_IGNORE, [0]*4, 0, 0, yaw, 0) #Отправляем относительное вращение
                event.wait(self.__mav_settings.mavlink_send_timeout)
                if event.is_set():
                    msg = self.msg_archive['GIMBAL_DEVICE_SET_ATTITUDE']['msg']
                    self.msg_archive['GIMBAL_DEVICE_SET_ATTITUDE']['is_read'].set()
                    return True
            return False
        finally:
            if 'GIMBAL_DEVICE_SET_ATTITUDE' in self.wait_msg:
                del self.wait_msg['GIMBAL_DEVICE_SET_ATTITUDE']

    def go_to_local_point(self, x, y):
        """ Поездка в точку с глобальными координатами """
        cmd_name = 'GO_TO_POINT'
        logging.info(
            f"[{self.name}] <Point> target: local point {{x:{x}, y:{y}}}...")
    ...
    def rotate(self, angle):
        """ Поворот робота относительно текущего положения """
        cmd_name = 'ROTATE'
        return self._send_yaw_orientation(angle)
    ...
    def go_to_local_point_body_fixed(self, x, y):
        ...
        logging.info(
            f"[{self.name}] <Point> target: body_fixed point {{x:{x}, y:{y}}}...")
        # _ _ _ _ yaw_rate yaw   force_set   afz afy afx   vz vy vx   z y x
        mask = 0b0000_10_0_111_111_000
        self._point_reached = False
        return self._send_position_target_local_ned(command_name=cmd_name,
    ...
    def get_local_position_lps(self, get_last_received: bool = True):
    ...
    def led_control(self, r=0, g=0, b=0, led_id=255):  # 255 all led
        ...
        if command:
            ...
            logging.info(f"{self.name} <LED> id: %s R: %i ,G: %i, B: %i send" % (
                led_id_print, r, g, b))
    ...
    def detect(item):
        pass    #Тут будет какой-то код, от организаторов,
                #Используйте функцию после распознавания метки, чтобы получить баллы
